...
function getMapsConfig(): MapsConfig {
  const directApiKey = process.env.VITE_GOOGLE_MAPS_API_KEY || process.env.GOOGLE_MAPS_API_KEY;
  const baseUrl = ENV.forgeApiUrl;
  const apiKey = ENV.forgeApiKey || directApiKey;
  ...
  throw new Error(
    "Google Maps credentials missing: set VITE_GOOGLE_MAPS_API_KEY or BUILT_IN_FORGE_API_KEY"
  );
}
